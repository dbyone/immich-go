...
| Immich API | ✅ 125/274 操作（45.6%）：auth/assets/albums/timeline/trash/search 全套(8 个长尾端点)/jobs/sessions 全套/api-keys/users 基础/server(无 license)/people 全套/memories 全套/**sync 增量化（update_id 水位 + 删除事件）**/duplicates/download/map/view/stacks 全套/partners 全套/config/public-config/onboarding 均可用；**shared-links 暂不实现**（设计决策） |
