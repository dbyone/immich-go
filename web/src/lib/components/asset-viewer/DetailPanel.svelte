<script lang="ts">
  ...
  // immich-go extension: re-run the server-side metadata pipeline (EXIF,
  // thumbnails, CLIP embedding, scene tags) for this one asset — the
  // backend half of MT Photos' per-photo refresh affordance.
  let isRefreshingAsset = $state(false);
  const refreshAsset = async () => {
    if (isRefreshingAsset) {
      return;
    }
    isRefreshingAsset = true;
    try {
      const response = await fetch(`/api/assets/${asset.id}/refresh`, {
        method: 'POST',
      ...
      // The pipeline runs asynchronously; give it a beat, then reload.
      await delay(2000);
      await assetViewerManager.setAssetId(asset.id);
    } catch (error) {
      handleError(error, $t('refresh'));
    } finally {
      isRefreshingAsset = false;
    }
  };
