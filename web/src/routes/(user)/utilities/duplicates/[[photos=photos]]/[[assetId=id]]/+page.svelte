<script lang="ts">
  ...
  // immich-go extension: ?exact=true keeps only byte-identical duplicates
  // (SHA-1 checksum groups) — MT Photos' MD5 filter.
  let exactOnly = $state(false);
  let isTogglingExact = $state(false);
  const loadDuplicates = async (exact: boolean) => {
    isTogglingExact = true;
    try {
      const response = await fetch(`/api/duplicates${exact ? '?exact=true' : ''}`, {
        credentials: 'same-origin',
      });
      if (!response.ok) {
        throw new Error(`Loading duplicates failed (${response.status})`);
      }
      duplicates = (await response.json()) as DuplicateResponseDto[];
    } catch (error) {
      handleError(error, 'Loading duplicates failed');
    } finally {
      isTogglingExact = false;
    }
  };
  const toggleExactOnly = async () => {
    exactOnly = !exactOnly;
    await loadDuplicates(exactOnly);
  };
...
<UserPageLayout title={data.meta.title + ` (${duplicates.length.toLocaleString($locale)})`} scrollbar={true}>
  {#snippet buttons()}
    <HStack gap={0}>
      <Button
        leadingIcon={mdiFileCompare}
        onclick={() => toggleExactOnly()}
        disabled={isTogglingExact}
        size="small"
        variant="ghost"
        color={exactOnly ? 'primary' : 'secondary'}
        title={$t('exact_duplicates_only')}
      >
        <Text class={exactOnly ? '' : 'hidden md:block'}>
          {$t('exact_duplicates_only')}
        </Text>
      </Button>
      <Button
